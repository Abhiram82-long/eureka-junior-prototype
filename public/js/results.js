...
function displayRecommendations(data) {
    ...
    // Ensure we always have summary content
    ...
    if (!summaryContent || summaryContent.trim() === '') {
        summaryContent = `Based on your requirements, we've identified ${data.recommendations.length} tools that match your needs. ` +
            `These recommendations are tailored specifically for your use case and budget constraints. Each tool has been ` +
            `analyzed for compatibility with your requirements and ranked by confidence score.`;
    }
    ...
    // Ensure we always have insights content
    let additionalContent = data.additionalNotes;
    if (!additionalContent || additionalContent.trim() === '') {
        additionalContent = `Consider trying free trials where available to test these tools before committing. ` +
            ...
            `When evaluating tools, consider factors like ease of use, integration capabilities, and long-term scalability for your needs.`;
    }
