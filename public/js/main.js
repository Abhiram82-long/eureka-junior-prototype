...
async function loadUserProfile() {
    try {
        ...
        if (response.ok) {
            ...
            if (apiKeyInput && userProfile.gemini_api_key) {
                apiKeyInput.value = userProfile.gemini_api_key;
            }
        }
    } catch (error) {
...
// Save search history
async function saveSearchHistory(formData) {
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Call Gemini API through backend if authenticated, otherwise direct
                let recommendations;
                if (isAuthenticated && userProfile?.gemini_api_key) {
                    recommendations = await getRecommendationsBackend(prompt);
                } else {
                    recommendations = await getRecommendations(prompt);
                }
...
// Backend API call for authenticated users
async function getRecommendationsBackend(prompt) {
    try {
